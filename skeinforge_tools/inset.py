...
from skeinforge_tools import preface
import cStringIO
import math
import os
import sys
...
def getChainGcode( fileName, gcodeText, insetPreferences = None ):
	"Inset the preface gcode text.  Chain inset the gcode if it is not already prefaced."
	if gcodeText == '':
		gcodeText = gcodec.getFileText( fileName )
	if not gcodec.isProcedureDone( gcodeText, 'preface' ):
		gcodeText = preface.getChainGcode( fileName, gcodeText )
	return getGcode( gcodeText, insetPreferences )

def getGcode( gcodeText, insetPreferences = None ):
...
def writeOutput( fileName = '' ):
	...
	insetGcode = getChainGcode( fileName, '', insetPreferences )
	if insetGcode == '':
		return
