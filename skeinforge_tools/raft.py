...
def getChainGcode( fileName, gcodeText, raftPreferences = None ):
	...
	if not gcodec.isProcedureDone( gcodeText, 'speed' ):
		gcodeText = speed.getChainGcode( fileName, gcodeText )
	return getGcode( gcodeText, raftPreferences )

def getGcode( gcodeText, raftPreferences = None ):
...
def writeOutput( fileName = '' ):
	...
	raftGcode = getChainGcode( fileName, '', raftPreferences )
	if raftGcode == '':
		return
...
class RaftPreferences:
	...
	def __init__( self ):
		...
		preferences.setHelpPreferencesFileNameTitleWindowPosition( self, 'skeinforge_tools.raft.html' )
