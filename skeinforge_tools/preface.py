...
def getChainGcode( fileName, gcodeText, prefacePreferences = None ):
	...
	if not gcodec.isProcedureDone( gcodeText, '"carve"' ): #"carve" is used instead of carve because quotes have to be used for strings in svg files
		gcodeText = carve.getGcode( fileName )
	return getGcode( gcodeText, prefacePreferences )

def getGcode( gcodeText, prefacePreferences = None ):
	"Preface the carves of a gcode text."
	if gcodeText == '':
		return ''
	if gcodec.isProcedureDone( gcodeText, 'preface' ):
		return gcodeText
	if prefacePreferences == None:
		prefacePreferences = PrefacePreferences()
		preferences.readPreferences( prefacePreferences )
	skein = PrefaceSkein()
	skein.parseGcode( prefacePreferences, gcodeText )
...
def writeOutput( fileName = '' ):
	...
	prefacePreferences = PrefacePreferences()
	preferences.readPreferences( prefacePreferences )
	print( 'File ' + gcodec.getSummarizedFilename( fileName ) + ' is being chain prefaced.' )
	suffixFilename = fileName[ : fileName.rfind( '.' ) ] + '_preface.gcode'
	prefaceGcode = getChainGcode( fileName, '', prefacePreferences )
	if prefaceGcode == '':
		return
	gcodec.writeFileText( suffixFilename, prefaceGcode )
	print( 'The prefaced file is saved as ' + suffixFilename )
	analyze.writeOutput( suffixFilename, prefaceGcode )
	print( 'It took ' + str( int( round( time.time() - startTime ) ) ) + ' seconds to preface the file.' )
...
class PrefacePreferences:
	...
	def __init__( self ):
		...
		preferences.setHelpPreferencesFileNameTitleWindowPosition( self, 'skeinforge_tools.preface.html' )
...
class PrefaceSkein:
	...
	def parseGcode( self, prefacePreferences, gcodeText ):
