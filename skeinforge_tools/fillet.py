...
tabs.  The functions writeOutput and getChainGcode check to see if the text has been combed, if not they call the
getChainGcode in oozebane.py to oozebane the text; once they have the oozebaned text, then they fillet.
...
def getChainGcode( fileName, gcodeText, filletPreferences = None ):
	...
	if not gcodec.isProcedureDone( gcodeText, 'oozebane' ):
		gcodeText = oozebane.getChainGcode( fileName, gcodeText )
	return getGcode( gcodeText, filletPreferences )

def getGcode( gcodeText, filletPreferences = None ):
...
def writeOutput( fileName = '' ):
	...
	filletGcode = getChainGcode( fileName, '', filletPreferences )
	if filletGcode == '':
		return
